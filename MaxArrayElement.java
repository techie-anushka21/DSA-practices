...
public class MaxArrayElement
{
    public static void main(String args[])
    {
        ...
        /*Finding the largest(max) element */
        int max= ar[0];    //Assume the 1st element at 0th index as max
        for(int i=0; i<size; i++)    //Use for loop so that every element is checked
        {
            if(ar[i]> max)    /*using if statement, check if any element is greater than 
                                the assumed max element */
            {
                max = ar[i];  /*if yes, then assign that element to the max variable */
            }
        }
